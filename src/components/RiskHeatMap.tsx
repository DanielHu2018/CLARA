  ...
  return (
    <div className="bg-black border border-zinc-900 rounded-lg p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-xl font-semibold text-white mb-1">Risk Heat Map</h3>
          <p className="text-base text-zinc-300">{ticker} - Likelihood × Impact Analysis</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 text-sm">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-zinc-300">Low ({severityCounts.low})</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-3.5 h-3.5 rounded-full bg-amber-400"></div>
              <span className="text-zinc-300">Medium ({severityCounts.medium})</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-3.5 h-3.5 rounded-full bg-red-400"></div>
              <span className="text-zinc-300">High ({severityCounts.high})</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 mb-4 pb-4 border-b border-zinc-900">
        ...
        <span className="text-base text-zinc-300 mr-2">Filter by COSO:</span>
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
              selectedCategories.includes(category.value)
                ...
                : 'bg-zinc-950 text-zinc-400 border border-zinc-900 hover:border-zinc-800 hover:text-zinc-200'
            }`}
          >
            {category.label}
          </button>
      ...
      <div className="bg-zinc-900/50 rounded-lg p-6">
        <div className="relative">
          {/* Y-axis label */}
          <div className="absolute -left-12 top-1/2 -translate-y-1/2 -rotate-90 text-sm text-zinc-300 whitespace-nowrap">
            Impact (%)
          </div>
                ...
                >
                  ...
                  {cell.count > 0 && (
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block z-10">
                      <div className="bg-black border border-zinc-700 rounded-lg p-2.5 shadow-xl min-w-[200px]">
          ...
          <div className="grid grid-cols-5 gap-1 mt-2">
            {[0, 20, 40, 60, 80].map((val, idx) => (
              <div key={idx} className="text-center text-sm text-zinc-300">
                {val}%
              </div>
            ))}
          </div>
          <div className="text-center text-sm text-zinc-300 mt-1">
            Likelihood (%)
          </div>
          ...
          <div className="absolute -left-8 top-0 bottom-12 flex flex-col justify-between text-right">
            {[100, 80, 60, 40, 20, 0].map((val, idx) => (
              <div key={idx} className="text-sm text-zinc-300 -translate-y-1/2">
                {val}%
              </div>
            ))}
          </div>
        ...
        {/* Legend */}
        <div className="mt-6 pt-4 border-t border-zinc-800 flex items-center justify-center gap-6 text-sm">
          <div className="flex items-center gap-2">
            <span className="text-zinc-400">Risk Density:</span>
          </div>
          <div className="flex items-center gap-2">
